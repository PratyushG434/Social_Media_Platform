...
const FeedContent = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-6">
      {/* Header */}
      <div className="sticky top-0 bg-white backdrop-blur-md border-b border-border/50 p-4 z-10 shadow-sm">
        ...
        <div className="relative">
          ...
          {showSearchResults && (
            <div className="absolute top-full left-0 w-full bg-white shadow-lg border border-gray-200 mt-2 rounded-xl z-50 max-h-64 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          {!loadingSuggestions && suggestedUsers.map((user, index) => (
            <div key={user.user_id} className="flex items-center justify-between p-3 bg-white/60 rounded-xl hover:bg-white/80 transition-all duration-300">
              ...
              <button onClick={() => handleFollow(user.user_id, index)} disabled={user.isFollowing} className={`text-white px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:scale-105 ${user.isFollowing ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:shadow-lg hover:shadow-primary/30'}`}>
