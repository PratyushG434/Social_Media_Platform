...
const storyExistsAndIsActive = async (storyId) => {
  const result = await db.query(
    "SELECT 1 FROM stories WHERE story_id = $1 AND expires_at > CURRENT_TIMESTAMP;",
    [storyId]
  );
  return result.rows.length > 0;
};
...
exports.getStoryById = async (storyId) => {
  const result = await db.query(`SELECT * FROM stories WHERE story_id = $1;`, [
    storyId,
  ]);
...
exports.deleteStory = async (storyId) => {
  await db.query(`DELETE FROM stories WHERE story_id = $1;`, [storyId]);
};
